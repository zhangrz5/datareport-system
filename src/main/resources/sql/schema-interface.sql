...
CREATE TABLE IF NOT EXISTS `t_key_certificate` (
    ...
    `sm2_public_key_decrypted` TEXT COMMENT 'SM2公钥(解密后明文)',
    `sm4_key_decrypted` TEXT COMMENT 'SM4密钥(解密后明文)',
...
CREATE TABLE IF NOT EXISTS `t_template_info` (
    ...
    `template_content_decrypted` TEXT COMMENT '模板内容(解密后)',
...
CREATE TABLE IF NOT EXISTS `t_supplement_task` (
    ...
    `task_content_decrypted` TEXT COMMENT '任务内容(解密后)',
    `download_time` DATETIME COMMENT '下载时间',
...
CREATE TABLE IF NOT EXISTS `t_notice_announcement` (
    ...
    `notice_content_decrypted` TEXT COMMENT '通知内容(解密后)',
