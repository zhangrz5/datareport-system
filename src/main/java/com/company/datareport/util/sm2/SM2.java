...
//	};
    ...
    public byte[] sm2GetZ(byte[] userId, ECPoint userKey)
    {
        ...
        p = ByteUtil.byteConvert32Bytes(userKey.getAffineXCoord().toBigInteger());
        sm3.update(p, 0, p.length);

        p = ByteUtil.byteConvert32Bytes(userKey.getAffineYCoord().toBigInteger());
        sm3.update(p, 0, p.length);
...
//		        kp = this.ecc_point_g.multiply(k);
                ...
                // 修改：使用 getAffineXCoord() 和 getAffineYCoord() 替代 getX() 和 getY()
                System.out.println("计算曲线点X1: " + kp.getAffineXCoord().toBigInteger().toString(16));
                System.out.println("计算曲线点Y1: " + kp.getAffineYCoord().toBigInteger().toString(16));
                System.out.println("");
    ...
    public void sm2Verify(byte md[], ECPoint userKey, BigInteger r, BigInteger s, SM2Result sm2Result)
    {
        ...
        else
        {
            ...
            // 修改：使用 getAffineXCoord() 和 getAffineYCoord() 替代 getX() 和 getY()
            System.out.println("计算曲线点X0: " + x1y1.getAffineXCoord().toBigInteger().toString(16));
            System.out.println("计算曲线点Y0: " + x1y1.getAffineYCoord().toBigInteger().toString(16));
            System.out.println("");

            x1y1 = x1y1.add(userKey.multiply(t));
            System.out.println("计算曲线点X1: " + x1y1.getAffineXCoord().toBigInteger().toString(16));
            System.out.println("计算曲线点Y1: " + x1y1.getAffineYCoord().toBigInteger().toString(16));
            System.out.println("");
            sm2Result.R = e.add(x1y1.getAffineXCoord().toBigInteger()).mod(ecc_n);
